...
const calculadoraSchema = z.object({
  valorImovel: z.string()
    ...
    .refine((val) => !isNaN(parseFloat(val)) && parseFloat(val) > 0, "Deve ser um número maior que zero"),
  entrada: z.string()
    .min(1, "Campo obrigatório")
    .refine((val) => !isNaN(parseFloat(val)) && parseFloat(val) >= 0, "Deve ser um número maior ou igual a zero"),
  taxaJuros: z.string()
    .min(1, "Campo obrigatório")
    .refine((val) => !isNaN(parseFloat(val)) && parseFloat(val) > 0, "Deve ser um número maior que zero"),
...
export default function CalculadoraRevisional() {
  ...
  const onSubmit = async (data: CalculadoraData) => {
    try {
      ...
      // Validar datas
      if (!data.vencimentoParcela || data.vencimentoParcela.trim() === "") {
        toast.error("Data de vencimento da primeira parcela é obrigatória");
        setIsLoading(false);
        return;
      }

      if (!data.dataContrato || data.dataContrato.trim() === "") {
        toast.error("Data de assinatura do contrato é obrigatória");
        setIsLoading(false);
        return;
      }
